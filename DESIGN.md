...
A 520–660px circle anchored `right: −80px → −40px`, vertically centered. The home hero uses the supplied five-color gradient as a texture: red `#AD4544` at the upper left, dark `#1F1E26` at the center, violet `#524153` near (73%, 27%), gold `#C7B472` at the upper right, and coral `#FF8B94` at the lower right. Within the orb, the light colors are compressed into dark and middle values; the corporate gradient on the hero and contact surfaces retains its original range. A WebGL sphere maps that texture onto curved normals, turns it once every 42 seconds, and adds a small flowing warp. The map reverses smoothly around the far side of the sphere so the image's unlike left and right edges never create a visible color seam. Subdued diffuse light, a restrained reflection, rim shading, and an offset cast shadow give it volume while the copy and light remain fixed. With JavaScript enabled, the surface fades in only after its first WebGL frame has rendered, preventing a visible swap from the differently positioned CSS gradient. Its animation phase is kept for the current tab across reloads so the color pattern does not jump back to the start. The CSS gradient remains visible without JavaScript or when WebGL is unavailable. Animation pauses offscreen and under reduced motion. Service detail heroes retain their family-specific static gradients (see `families[].gradient`).
